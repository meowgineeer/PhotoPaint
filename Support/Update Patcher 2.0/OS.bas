...
'
'Newer Windows versions expose some neat features (like progress bars overlaying the taskbar), and PhotoPaint
...
Private Function AppProcessID() As Long
    ...
    ElseIf (Not m_TriedToRetrieveID) Then
        ...
        'Prep a process enumerator.  We're going to search the active process list, looking for PhotoPaint.
        ...
        If (hSnapShot <> INVALID_HANDLE_VALUE) Then
            ...
            If (ProcessFirst(hSnapShot, VarPtr(uProcess)) <> 0) Then
                ...
                ' (NOTE!  Inside the IDE, this only works if PhotoPaint is the only running VB6 window.
        ...
        If (numArgs > 0) Then
            ...
            For i = 0 To numArgs - 1
                ...
                If removeExePath Then
                    If (InStr(1, tmpString, "PhotoPaint.exe", vbBinaryCompare) = 0) Then dstStringStack.AddString tmpString
                Else
...
Public Function IsAeroAvailable() As Boolean
    ...
    If (m_ThemingAvailable = pdta_Unknown) Then
        ...
        Else
            ...
            sClass = "Window"
            hTheme = OpenThemeData(FormPatch.hWnd, StrPtr(sClass))
            If (hTheme <> 0) Then
                m_ThemingAvailable = pdta_True
...
Public Function SetTaskbarProgressState(ByVal tbpFlags As PD_TaskBarProgress) As Long
    If WIN7_FEATURES_ALLOWED Then SetTaskbarProgressState = CallInterface(m_taskbarObjHandle, SetProgressState_, 2, FormPatch.hWnd, tbpFlags)
End Function
...
Public Function SetTaskbarProgressValue(ByVal amtCompleted As Long, ByVal amtTotal As Long) As Long
    If WIN7_FEATURES_ALLOWED Then
        ...
        Else
            ...
            SetTaskbarProgressValue = CallInterface(m_taskbarObjHandle, SetProgressValue_, 5, FormPatch.hWnd, amtCompleted, 0, amtTotal, 0)
        End If
    End If
End Function
...
'Return a unique session ID for this PhotoPaint instance.  A session ID is generated by retrieving a random GUID,
