...
'Internal Windows Private Constants that define paper sizes.  As paper size text may be localized,
' it can't be relied upon for matching standard image sizes.  1/10th mm differences in reported
' sizes for standard media also makes it unreliable.  These consts could hypothetically be used to
' absolutely match up a given paper size with a standard size, but PD doesn't use this at present.
...
Public Sub PrintViaWindowsPhotoPrinter()
    ...
    Saving.QuickSaveDIBAsPNG tmpFilename, tmpDIB, True, True
    Set tmpDIB = Nothing
