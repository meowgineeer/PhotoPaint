...
Public Sub ResetAppIcons()
    ...
    iconPath = App.Path & "\Gimp.ico"
    
    Dim fileExists As Boolean
    On Error Resume Next
    fileExists = (Dir$(iconPath) <> "")
    On Error GoTo 0
    ...
    If fileExists Then
        If (m_DefaultIconLarge = 0) Then
            m_DefaultIconLarge = LoadImageW(0&, StrPtr(iconPath), IMAGE_ICON, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON), LR_LOADFROMFILE)
        End If
        If (m_DefaultIconSmall = 0) Then
            m_DefaultIconSmall = LoadImageW(0&, StrPtr(iconPath), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_LOADFROMFILE)
        End If
    Else
        If (m_DefaultIconLarge = 0) Then
